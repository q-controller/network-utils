...
func CreateTapWithManager(mgr LinkManager, name string, bridgeName string) error {
	...
	if existsErr != nil {
		return fmt.Errorf("unexpected error checking link: %w", existsErr)
	}
	if !exists {
		if err := mgr.AddLink(name, LinkTypeTap); err != nil {
			return fmt.Errorf("failed to add tap device %s: %w", name, err)
		}
		if err := mgr.SetMaster(name, bridgeName); err != nil {
			if delErr := mgr.DeleteLink(name); delErr != nil {
				return fmt.Errorf("failed to set master for tap %s: %w, failed to delete tap: %w", name, err, delErr)
			}
			return fmt.Errorf("failed to set master for tap %s: %w", name, err)
		}
	}

	if err := mgr.BringUp(name); err != nil {
		if delErr := mgr.DeleteLink(name); delErr != nil {
			return fmt.Errorf("failed to bring tap %s up: %w, failed to delete tap: %w", name, err, delErr)
		}
		return fmt.Errorf("failed to bring tap %s up: %w", name, err)
	}
